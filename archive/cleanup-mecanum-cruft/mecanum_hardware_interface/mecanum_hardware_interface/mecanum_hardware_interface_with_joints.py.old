...
class MecanumHardwareInterface(Node):
    ...
    def cmd_vel_callback(self, msg):
        ...
        m1 = max(-50, min(50, int(v_fl * factor)))
        m2 = max(-50, min(50, int(-v_fr * factor)))  # Note: negated
        m3 = max(-50, min(50, int(-v_rl * factor)))  # Note: negated
        m4 = max(-50, min(50, int(v_rr * factor)))
